...
//            ]);
     ...
     * 拼团管理订单列表（团长）
     */
    public function getsettlementstatisticsList()
    {
        if (!$this->validate())
            return $this->getModelError();
        $query = PtOrder::find()
            ...
            ->andWhere(['o.is_delete'=>0,'o.store_id'=>$this->store_id,'o.is_settlementstatistics'=>1,'o.parent_id'=>0])
            ->andWhere(['o.is_pay'=>1])
            ->leftJoin(['od'=>PtOrderDetail::tableName()],'od.order_id=o.id')
            ->leftJoin(['g'=>PtGoods::tableName()],'g.id=od.goods_id')
            ->leftJoin(['u'=>User::tableName()],'u.id=o.user_id');
     ...
     * 拼团管理订单列表（团长）
     */
    public function getsettlementstatisticsInfo($pid = 0)
    {
        $query = PtOrder::find()
            ->alias('o')
            ->select([
            ...
            ->andWhere(['o.is_delete'=>0,'o.store_id'=>$this->store_id,'o.is_settlementstatistics'=>1])
