...
class PtGoodsForm extends Model
{
    ...
    public $settlementstatisticstime;
    public $attr;
    public $service;
    public $sort;
    ...
    public $freight;
    public $unit;
    public $settlementstatistics_num;
    public $limit_time;
    public $is_only;
    public $is_more;
    public $colonel;
    ...
    {
        return [
            [['store_id', 'name', 'original_price', 'price', 'detail', 'settlementstatistics_num', 'settlementstatisticstime', 'cat_id'], 'required'],
            [['store_id', 'cat_id', 'settlementstatisticstime', 'sort', 'virtual_sales', 'weight', 'freight', 'settlementstatistics_num', 'limit_time', 'is_only', 'is_more', 'buy_limit', 'type'], 'integer'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'settlementstatisticstime' => '拼团时间/小时',
            'attr' => '规格的库存及价格',
            'service' => '服务选项',
            'sort' => '商品排序 升序',
            'virtual_sales' => '虚拟销量',
            ...
            'addtime' => '添加时间',
            'is_delete' => '是否删除',
            'settlementstatistics_num' => '商品成团数',
            'goods_pic_list' => '商品图集',
            'is_only' => '是否允许单独购买',
            'limit_time' => '拼团限时',
            'is_more' => '是否允许多件购买',
    ...
    private function setAttr($goods)
    {
        ...
        foreach ($this->attr as $i => $item) {
            ...
            foreach ($item['attr_list'] as $a) {
                $attr_settlementstatistics_model = Attrsettlementstatistics::findOne(['store_id' => $this->store_id, 'attr_settlementstatistics_name' => $a['attr_settlementstatistics_name'], 'is_delete' => 0]);
                if (!$attr_settlementstatistics_model) {
                    $attr_settlementstatistics_model = new Attrsettlementstatistics();
                    $attr_settlementstatistics_model->attr_settlementstatistics_name = $a['attr_settlementstatistics_name'];
                    $attr_settlementstatistics_model->store_id = $this->store_id;
                    $attr_settlementstatistics_model->is_delete = 0;
                    $attr_settlementstatistics_model->save();
                }
                $attr_model = Attr::findOne(['attr_settlementstatistics_id' => $attr_settlementstatistics_model->id, 'attr_name' => $a['attr_name'], 'is_delete' => 0]);
                if (!$attr_model) {
                    $attr_model = new Attr();
                    $attr_model->attr_name = $a['attr_name'];
                    $attr_model->attr_settlementstatistics_id = $attr_settlementstatistics_model->id;
