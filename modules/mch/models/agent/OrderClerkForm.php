...
class OrderClerkForm extends Model
{
    ...
    public function save()
    {
        ...
        $list = Setting::findOne(['store_id' => $this->store_id]);
        //记录日志
        $hld = 0;
        $coupon = intval($order->return_integral);
        $integral =  intval($order->return_integral);

//        var_dump($integral);die;
        $integralLog = new IntegralLog();
        $integralLog->user_id = $user->id;
        //卖优惠券
        $integralLog->content = "（申请代理商：" . $user->nickname . " 欢乐豆" . $user->hld . "：" . $hld . " 豆" . " 优惠券" . $user->coupon . "：" . $coupon . " 张" . $integral . '积分）';
