...
class OrderForm extends Model
{
    ...
    public $cat_id; //查询哪一种奖励
    public $cat_id_is_open; //是否打开查询种类

    public function rules()
    {
        return [
            [['keyword', 'date_start', 'date_end'], 'trim'],
            [['cat_id','status', 'keyword_1'], 'integer'],
...
//        }
        ...
        foreach ($list AS $k => $v) {
            $list[$k]['orderFrom'] = \app\modules\api\models\settlementbonus\OrderForm::find()
                ->select([
                    'key', 'value'
                ])
