...
class ZcGoodsForm extends Model
{
    public $goods;
    ...
    public function getList($store_id)
    {
        $query = ZcGoods::find()
            ...
            ->leftJoin('{{%zc_cat}} c', 'g.cat_id=c.id');
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            if ($goods->save()) {
                ZcGoodsPic::updateAll(['is_delete' => 1], ['goods_id' => $goods->id]);
                foreach ($this->goods_pic_list as $pic_url) {
                    $goods_pic = new ZcGoodsPic();
                ...
                ZcForm::updateAll(['is_delete' => 1], ['goods_id'=>$goods->id]);
                foreach ($this->form_list AS $form){
                    $form_list = new ZcForm();
