                ...
                <form method="get">
                    ...
                    <div flex="dir:left">
                        <div class="mr-4">
                            ...
                            <?php endif; ?>
                            <?php if ($shop): ?>
                                <span class="status-item mr-3">智能鲜蜂服务点：<?= $shop->name ?>的订单</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </form>
            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(['crowdc/order/index']) ?>">全部</a>
                </li>

                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 0])) ?>">未付款<?= isset($store_data['status_count']['status_0']) ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>
                ...
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 8 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 8])) ?>">待支付余款<?= isset($store_data['status_count']['status_0']) ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>
                ...
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 9 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 9])) ?>">失败<?= isset($store_data['status_count']['status_0']) ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>
                ...
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 1])) ?>">待发货<?= isset($store_data['status_count']['status_1']) ? '(' . $store_data['status_count']['status_1'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 2])) ?>">待收货<?= isset($store_data['status_count']['status_1']) ? '(' . $store_data['status_count']['status_1'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $status == 3 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 3])) ?>">已完成<?= isset($store_data['status_count']['status_3']) ? '(' . $store_data['status_count']['status_3'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $status == 5 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['crowdc/order/index'], $condition, ['status' => 5])) ?>">已取消<?= isset($store_data['status_count']['status_5']) ? '(' . $store_data['status_count']['status_5'] . ')' : null ?></a>
                </li>
            </ul>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="order-tab-5">
                                ...
                                <?php if ($order_item['is_delete'] == 0): ?>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-primary apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/crowdc/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意请求</a>
                                    </div>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-danger apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/crowdc/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝请求</a>
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                            ...
                            <a class="btn btn-sm btn-primary"
                               href="<?= $urlManager->createUrl(['mch/crowdc/order/detail', 'order_id' => $order_item['id']]) ?>">详情</a>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="5">
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/crowdc/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createAbsoluteUrl(['mch/crowdc/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
...
<script>
    ...
    $(document).on('click', '.add-price', function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/crowdc/order/add-price'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                order_id: order_id,
    ...
    $(document).on('click', '.add-yukuan', function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/crowdc/order/clerk'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                order_id: order_id,
