...
<style>
    ...
    .open-date-delete-crowdc:active {
        background: #da3b3a;
        text-decoration: none;
    }


    .open-date-delete-crowdc {
        display: inline-block;
        color: #fff;
        font-weight: bolder;
        background: #ff6461;
        ...
        <form class="auto-form" method="post">
            <div class="form-group row">
                <div class="form-group-label col-sm-2 text-right">
                    <label class="col-form-label required">众筹商品</label>
                </div>
                <div class="col-sm-6">
                    ...
                    <template v-else>
                        <table class="table table-bordered">
                            ...
                            <tr v-for="attr_row in goods.attr">
                                ...
                                <td><input style="width: 4rem" type="number"
                                           class="form-control form-control-sm check-goods-data"
                                           v-model="attr_row.seckill_coupon"
                                           step="1" min="0"
                                           v-bind:max="attr_row.coupon==0?goods.price:attr_row.coupon"></td>
<!--                                <td><input style="width: 4rem" type="number"-->
<!--                                           class="form-control form-control-sm check-goods-data"-->
<!--                                           v-model="attr_row.seckill_integral_buy"-->
<!--                                           step="1" min="0"-->
<!--                                           v-bind:max="attr_row.integral_buy==0?goods.price:attr_row.integral_buy"></td>-->
                                <td>{{attr_row.num}}</td>
                                <td><input style="width: 4rem" type="number"
                                           class="form-control form-control-sm check-goods-data"
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    ...
                    <?php endforeach; else: ?>
                        <div class="text-muted">商城未设置众筹开放时间，请<a
                                    href="<?= $urlManager->createUrl(['mch/crowdc/seckill/index']) ?>">前往设置</a></div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="form-group row">
                <div class="form-group-label col-sm-2 text-right">
                    <label class="col-form-label required">众筹开始日期</label>
                </div>
                <div class="col-sm-6">
                    <span v-for="(item,index) in begin_data" class="mr-3 badge badge-default">
                        <span>{{item}}</span>
                        <a href="javascript:" class="open-date-delete-crowdc" v-bind:index="index">×</a>
                    </span>
<!--                    <a class="badge badge-primary" href="javascript:" id="quickAddDatebegin">+&nbsp;添加</a>-->
                    <a class="badge badge-primary" href="javascript:" data-toggle="modal"
                       data-target="#addDateModal-crowdc">+&nbsp;开始结束日期添加</a>
                    <div class="text-muted fs-sm mt-1">只可选择今日起一个月内的时间</div>
                </div>
            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="<?= $urlManager->createUrl(['mch/crowdc/seckill/goods-search']) ?>"
        ...
        <!-- Modal -->
        <div class="modal fade" data-backdrop="static" id="addDateModal-crowdc" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">添加众筹日期</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-6 mb-2">
                                <input value="<?= date('Y-m-d') ?>" id="date_timepicker_start-crowdc" class="form-control">
                            </div>
                            <div class="col-6 mb-2">
                                <input id="date_timepicker_end-crowdc" class="form-control">
                            </div>
                        </div>
                        <div class="text-muted fs-sm mt-1">只可选择今日起一个月内的时间</div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">取消</button>
                        <button type="button" class="btn btn-primary add-date-confirm-crowdc">确认</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('#date_timepicker_start-crowdc').datetimepicker({
        format: 'Y-m-d',
        minDate: "<?=date('Y-m-d')?>",
        maxDate: "<?=date('Y-m-d', strtotime('+1 month'))?>",
        onShow: function (ct) {
            this.setOptions({
                maxDate: $('#date_timepicker_end-crowdc').val() ? $('#date_timepicker_end-crowdc').val() : "<?=date('Y-m-d', strtotime('+1 month'))?>"
            })
        },
        timepicker: false
    });
    $('#date_timepicker_end-crowdc').datetimepicker({
        format: 'Y-m-d',
        minDate: "<?=date('Y-m-d')?>",
        maxDate: "<?=date('Y-m-d', strtotime('+1 month'))?>",
        onShow: function (ct) {
            this.setOptions({
                minDate: $('#date_timepicker_start-crowdc').val() ? $('#date_timepicker_start-crowdc').val() : false
            })
        },
        timepicker: false
    });
    ...
    $(document).on("click", ".add-date-confirm-crowdc", function () {
        var start_date = $("#date_timepicker_start-crowdc").val();
        var end_date = $("#date_timepicker_end-crowdc").val();
        var list = getDateList(start_date, end_date);
        for (var i in list) {
            var date = list[i];
            var in_array = false;
        ...
        app.begin_data.sort();
        $("#addDateModal-crowdc").modal("hide");
    ...
    $(document).on("click", ".open-date-delete-crowdc", function () {
        var index = $(this).attr("index");
    ...
    $(document).on("click", ".seckill-submit-btn", function () {
        ...
        $.ajax({
            ...
            data: {
                ...
                start_date_crowdc: $("#date_timepicker_start-crowdc").val(),
                end_date_crowdc: $("#date_timepicker_end-crowdc").val(),
            },
            success: function (res) {
                if (res.code == 0) {
                    $.alert({
