        ...
        <table class="table table-bordered bg-white">
            ...
            <?php foreach ($list as $u): ?>
                <tr>
                    ...
                    <td>
                        <a href="<?= $urlManager->createUrl(['mch/financial/card', 'clerk_id' => $u['id']]) ?>"><?= $u['card_count'] ?></a>
                    </td>
                    <td>
                        <?php if ($u['is_clerk'] == 0): ?>
                            <a class="btn btn-sm btn-primary del" href="javascript:"
                               data-url="<?= $urlManager->createUrl(['mch/financial/clerk-edit', 'id' => $u['id'], 'status' => 1]) ?>"
                               data-content="是否设为核销员？">设为核销员</a>
                        <?php else: ?>
                            <a class="btn btn-sm btn-danger del" href="javascript:"
                               data-url="<?= $urlManager->createUrl(['mch/financial/clerk-edit', 'id' => $u['id'], 'status' => 0]) ?>"
                               data-content="是否解除核销员">解除核销员</a>
                        <?php endif; ?>
                        <a class="btn btn-primary btn-sm user-shop-edit" data-id="<?= $u['id'] ?>"
                           href="javascript:">修改智能鲜蜂服务点</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </table>
                        ...
                        <div class="modal-body">
                            <div class="form-group row">
                                <div class="col-3 text-right">
                                    <label class="col-form-label required">智能鲜蜂服务点选择</label>
                                </div>
                                <div class="col-9">
                                    ...
                                    <template v-else>
                                        <label class="col-form-label">暂未设置智能鲜蜂服务点，<a
                                        ...
                                        <tr v-for="(item,index) in show_user_list">
                                            ...
                                            <td>
                                                <a class="btn btn-primary btn-sm send" href="javascript:"
                                                   data-url="<?= $urlManager->createUrl(['mch/financial/clerk-edit', 'status' => 1]) ?>"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a class="btn btn-primary send" href="javascript:"
                               data-url="<?= $urlManager->createUrl(['mch/financial/clerk-edit', 'status' => 1, 'edit' => 1]) ?>"
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            showKeyword: function () {
                ...
                $.ajax({
                    url: '<?=$urlManager->createUrl(['mch/financial/get-user'])?>',
                    dataType: 'json',
                    type: 'get',
                    data: {
                        keyword: keyword
...
<script>
    ...
    $(document).on('click', '.send', function () {
        ...
        if (app.select_shop == "") {
            $('.shop-error').prop('hidden', false).html('请先选择智能鲜蜂服务点');
            return;
        }
        $.ajax({
            url: a.data('url'),
