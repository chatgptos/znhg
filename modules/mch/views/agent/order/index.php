            ...
            <div class="p-4 bg-shaixuan">
                <form method="get">
                    ...
                    <div flex="dir:left">
                        <div class="mr-4">
                            <?php if (isset($user)): ?>
                                <span class="status-item mr-3">会员：<?= $user->nickname ?>的记录</span>
                            <?php endif; ?>
                            <?php if (isset($clerk)): ?>
                                <span class="status-item mr-3">通过员：<?= $clerk->nickname ?>的记录</span>
                            <?php endif; ?>
                            <?php if (isset($shop)): ?>
                                <span class="status-item mr-3">智能鲜蜂服务点：<?= $shop->name ?>的记录</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </form>
            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/agent/order/index'])) ?>">全部</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/agent/order/index'], $condition, ['status' => 0])) ?>">待申请</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/agent/order/index'], $condition, ['status' => 1])) ?>">待审核</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/agent/order/index'], $condition, ['status' => 2])) ?>">已通过审核</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 3 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/agent/order/index'], $condition, ['status' => 3])) ?>">未通过</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 5 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/agent/order/index'], $condition, ['status' => 5])) ?>">已取消</a>
                </li>
            </ul>
        </div>
        <table class="table table-bordered bg-white">
            <tr>
                <th class="order-tab-1">代理商信息</th>
                <th class="order-tab-2">金额</th>
                <th class="order-tab-3">实际支付</th>
                <th class="order-tab-3">活动结束返回用户</th>
                <th class="order-tab-4">记录状态</th>
                ...
                <table class="table table-bordered bg-white">
                    <tr>
                        <td colspan="5">
                            ...
                            <span class="mr-5">代理商编号：<?= $order_item['order_no'] ?></span>
                    ...
                    <tr>
                        ...
                        <td class="order-tab-3">
                            <div><?= $order_item['level'] ?>等级</div>
                            <div><?= $order_item['pay_price'] ?>元</div>
                            <div><?= $order_item['coupon'] ?>券</div>
                        ...
                        <td class="order-tab-4">
                            ...
                            <?php if ($order_item['is_pay'] == 1): ?>
                                <div>
                                    ...
                                    <?php else: ?>
                                        <span class="badge badge-default">未通过</span>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
                        ...
                        <td class="order-tab-5">
                            ...
                            <?php if ($order_item['is_pay'] == 1 && $order_item['is_confirm'] != 1&& $order_item['is_use'] != 1): ?>
                                <a class="btn btn-sm btn-primary update" href="javascript:" data-toggle="modal"
                                   data-target="#price" data-id="<?= $order_item['id'] ?>">通过审核</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                </table>
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/agent/order/refund'])?>",
            type: "get",
...
<!--                <a href="javascript:" class="btn btn-primary add-price" data-type="2">代理商</a>-->
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on('click', '.add-price', function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/agent/order/clerk'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                order_id: order_id,
