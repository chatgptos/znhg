...
$this->params['active_nav_settlementstatistics'] = 2;
$returnUrl = Yii::$app->request->referrer;
if (!$returnUrl)
    $returnUrl = $urlManager->createUrl(['mch/settlementstatistics/goods/goods']);
?>
<style>
    .goods-pic {
        ...
        <form class="form auto-form" method="post" autocomplete="off" return="<?= $returnUrl ?>">
            <div class="form-body">
                <div class="form-settlementstatistics row">
                    <div class="form-settlementstatistics-label col-3 text-right">
                ...
                <template v-if="attr_settlementstatistics_list && attr_settlementstatistics_list.length>0">
                    <div class="form-settlementstatistics row" v-for="(attr_settlementstatistics,index) in attr_settlementstatistics_list">
                        <div class="col-3 text-right">
                            <input class="attr-checkbox-all checkedAll" v-bind:data-index="index" type="checkbox">
                            <label class="">{{attr_settlementstatistics.attr_settlementstatistics_name}}</label>
                        </div>
                        <div class="col-9">
                            <label v-for="attr in attr_settlementstatistics.attr_list" class="mr-4">
...
<script>
    ...
    var page = new Vue({
        ...
        data: {
            //所有规格
            attr_settlementstatistics_list: <?=json_encode($attr_settlementstatistics_list)?>,
    ...
    $(document).on("change", ".attr-checkbox", function () {
        var attr_settlementstatistics_list = [];
        for (var i in page.attr_settlementstatistics_list) {
            for (var j in page.attr_settlementstatistics_list[i].attr_list) {
                if (page.attr_settlementstatistics_list[i].attr_list[j].checked) {
                    if (!attr_settlementstatistics_list[i])
                        attr_settlementstatistics_list[i] = [];
                    var object = {
                        attr_id: page.attr_settlementstatistics_list[i].attr_list[j].attr_id,
                        attr_name: page.attr_settlementstatistics_list[i].attr_list[j].attr_name,
                    };
                    attr_settlementstatistics_list[i].push(object);
                }
            }
        }
        console.log(attr_settlementstatistics_list);

        page.checked_attr_list = getAttrList(attr_settlementstatistics_list);
    });

    $(document).on("change", ".checkedAll", function () {
        var index = $(this).data('index');
        var row_info = page.attr_settlementstatistics_list[index].attr_list;
        if ($(this).is(':checked')) {
            for (var i in row_info) {
                page.attr_settlementstatistics_list[index].attr_list[i].checked = true;
            }
        } else {
            for (var i in row_info) {
                page.attr_settlementstatistics_list[index].attr_list[i].checked = false;
            }
        }
    ...
    function getAttrList(attr_settlementstatistics_list) {
        var array = attr_settlementstatistics_list;
