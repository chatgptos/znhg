            ...
            <div class="p-4 bg-shaixuan">
                <form method="get">
                    ...
                    <div flex="dir:left">
                            ...
                            <div class="form-group row w-20">
                                <div class="col-5">
                                    <select class="form-control" name="keyword_1">
                                        ...
                                        <!--                                        <option value="2" -->
                                        <? //= isset($Gets['keyword_1']) && $Gets['keyword_1'] == 2 ? "selected" : "" ?><!-->
                                        -->
                                        <!--                                            用户-->
                                        <!--                                        </option>-->
                                        <!--                                        <option value="3" -->
                                        <? //= isset($Gets['keyword_1']) && $Gets['keyword_1'] == 3 ? "selected" : "" ?><!-->
                                        -->
                                        <!--                                            收货人-->
                                        <!--                                        </option>-->
                                    </select>
                        ...
                        <!--                                               autocomplete="off"-->
                        <!--                                               value="-->
                        <? //= isset($Gets['date_start']) ? trim($Gets['date_start']) : '' ?><!--">-->
                        ...
                        <!--                                               autocomplete="off"-->
                        <!--                                               value="-->
                        <? //= isset($Gets['date_end']) ? trim($Gets['date_end']) : '' ?><!--">-->
                        <!--                                        <span class="input-group-btn">-->
                    ...
                    <div flex="dir:left">
                        <div class="mr-4">
                            <div class="form-group"><?=Yii::$app->request->url?>
                    ...
                    <div flex="dir:left">
                        <div class="mr-4">
                            <?php if ($user): ?>
                                <span class="status-item mr-3">会员：<?= $user->nickname ?>的货号</span>
                            <?php endif; ?>
                            <?php if ($clerk): ?>
                                <span class="status-item mr-3">核销员：<?= $clerk->nickname ?>的货号</span>
                            <?php endif; ?>
                            <?php if ($shop): ?>
                                <span class="status-item mr-3">门店：<?= $shop->name ?>的货号</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </form>
            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(['itemno/index']) ?>">全部</a>
                </li>

                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['itemno/index'], $condition, ['status' => 0])) ?>">未扫码<?= isset($store_data['status_count']['status_0']) ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>

                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['itemno/index'], $condition, ['status' => 1])) ?>">待打印证书<?= isset($store_data['status_count']['status_1']) ? '(' . $store_data['status_count']['status_1'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['itemno/index'], $condition, ['status' => 2])) ?>">待选择新货号<?= isset($store_data['status_count']['status_2']) ? '(' . $store_data['status_count']['status_2'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $status == 3 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['itemno/index'], $condition, ['status' => 3])) ?>">已完成<?= isset($store_data['status_count']['status_3']) ? '(' . $store_data['status_count']['status_3'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $status == 5 ? 'active' : null ?>"
                       href="<?= yii\helpers\Url::to(array_merge(['itemno/index'], $condition, ['status' => 5])) ?>">已取消<?= isset($store_data['status_count']['status_5']) ? '(' . $store_data['status_count']['status_5'] . ')' : null ?></a>
                </li>
            </ul>
        </div>
        <table class="table table-bordered bg-white">
            <tr>
                <th class="order-tab-1">旧货号</th>
                <th class="order-tab-5">新货号</th>
                <!--                <th class="order-tab-3">实际付款</th>-->
                <th class="order-tab-5">时间</th>
                <th class="order-tab-1">操作</th>
            </tr>
        </table>
        <?php foreach ($list as $order_item): ?>
            <div class="order-item">
                <table class="table table-bordered bg-white">
                    <!--                    <tr>-->
                    <!--                        <td colspan="5">-->
                    <!--                            <span class="mr-5">时间</span>-->
                    <!--                            <span class="mr-5">货号</span>-->
                    <!--                        </td>-->
                    <!--                    </tr>-->
                    <tr>
                        <td colspan="1">
                            <span class="mr-1"><?= $order_item['order_no'] ?></span>
                        </td>
                        <td colspan="5">
                            <span class="mr-5"><?= $order_item['express_no'] ?></span>
                        </td>
                        <td colspan="1">
                            <span class="mr-1"><?= date('Y-m-d H:i:s', $order_item['addtime']) ?></span>
                        </td>
                        ...
                        <td class="order-tab-5">
                            <?php if ($order_item['is_pay'] == 0 && $order_item['is_cancel'] == 0): ?>
                                <a class="btn btn-sm btn-primary update" href="javascript:" data-toggle="modal"
                                   data-target="#price" data-id="<?= $order_item['id'] ?>">打印证书修改</a>
                            <?php endif; ?>
                            <?php if ($order_item['apply_delete'] == 1): ?>
                                <?php if ($order_item['is_delete'] == 0): ?>
                                    <div class="mb-2">
                            ...
                            <?php else: ?>
                                <?php if ($order_item['is_pay'] == 1 && $order_item['is_confirm'] != 1): ?>
                                    <a class="btn btn-sm btn-primary send-btn" href="javascript:"
                                       data-order-id="<?= $order_item['id'] ?>"><?= ($order_item['is_send'] == 1) ? "修改新货号" : "打印证书" ?></a>
                                <?php endif; ?>
                            <?php endif; ?>
                            <a class="btn btn-sm btn-primary"
                               href="<?= $urlManager->createUrl(['mch/itemno/detail', 'order_id' => $order_item['id']]) ?>">详情</a>
                        </td>
                    </tr>
                    <!--<tr>
                        <td colspan="5">
                        ...
                        <form class="send-form" method="post">
                                ...
                                <div class="col-9">
                                    <div class="pt-1">
                                        <label class="custom-control custom-radio">
                                            ...
                                            <span class="custom-control-description">新证书货号</span>
                                        </label>
                                        <label class="custom-control custom-radio">
                                            <input id="radio2" value="0" name="is_express" type="radio"
                                                   class="custom-control-input is-express">
                                            <span class="custom-control-indicator"></span>
                                            <span class="custom-control-description">无需证书无需新货号</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                            <div class="is-true-express">
                                ...
                                <div class="input-group mb-3">
                                    <input class="form-control" placeholder="请输入证书品牌厂家" type="text" autocomplete="off"
                                           name="express">
...
<!--                                       name="post_code">-->
                                <label><a class="print" href="javascript:">打印合格证书</a></label>
                                <label><a href='http://www.c-lodop.com/download.html' target='_blank'>下载插件</a></label>
                                <div class="text-danger">需要设置证书打印功能</div>
                                <label>新货号</label>
...
<!--                <input class=" form-control money" type="number" placeholder="请填写增加或减少的价格">-->
                ...
                <label><a class="print" href="javascript:">打印合格证模版</a></label>
                <label><a href='http://www.c-lodop.com/download.html' target='_blank'>下载插件</a></label>
                <div class="text-danger">需要设置证书打印功能</div>
            </div>
            <div class="modal-footer">
                <a href="javascript:" class="btn btn-primary add-price" data-type="1">制作</a>
                <a href="javascript:" class="btn btn-primary add-price" data-type="2">跳过</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createAbsoluteUrl(['mch/itemno/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
