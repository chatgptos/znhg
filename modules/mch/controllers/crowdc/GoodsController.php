...
use app\models\Card;
use app\modules\mch\models\crowdc\Cat;
use app\modules\mch\models\crowdc\Goods;
use app\modules\mch\models\crowdc\GoodsForm;
use app\modules\mch\models\crowdc\GoodsCard;
use app\modules\mch\models\crowdc\CatForm;
...
class GoodsController extends Controller
{
    ...
    public function actionGoodsUpDown($id = 0, $type = 'down')
    {
        if ($type == 'down') {
            ...
            if (!$goods->getNum()) {
                $return_url = \Yii::$app->urlManager->createUrl(['mch/crowdc/goods/goods-attr', 'id' => $goods->id]);
                if (!$goods->use_attr)
                    $return_url = \Yii::$app->urlManager->createUrl(['mch/crowdc/goods/goods-edit', 'id' => $goods->id]) . '#step3';
                $this->renderJson([
                    'code' => 1,
                    'msg' => '商品库存不足，请先完善商品库存',
